...
This includes not just casino employees, but workers throughout the supply chain and the broader community who benefit when those workers spend their paychecks locally.`;
  ...
  // Subtotal row
  const taxSubtotalDirect = (gamingTaxResult?.amount || 0) + results.totals.tax.direct;
  const taxSubtotalTotal = (gamingTaxResult?.amount || 0) + results.totals.tax.total;
  taxDataRows.push([
    { text: 'Subtotal', options: { bold: true } },
    { text: formatCurrency(taxSubtotalDirect), options: { bold: true } },
    { text: formatCurrency(results.totals.tax.indirect), options: { bold: true } },
    { text: formatCurrency(results.totals.tax.induced), options: { bold: true } },
    { text: formatCurrency(taxSubtotalTotal), options: { bold: true } }
  ]);

  // Placeholder rows for future tax types
  taxDataRows.push([
    { text: 'Payroll Taxes', options: { color: COLORS.grayText, italic: true } },
    { text: '-', options: { color: COLORS.grayText, italic: true } },
    { text: '-', options: { color: COLORS.grayText, italic: true } },
    { text: '-', options: { color: COLORS.grayText, italic: true } },
    { text: '-', options: { color: COLORS.grayText, italic: true } }
  ]);
  taxDataRows.push([
    { text: 'Income Taxes', options: { color: COLORS.grayText, italic: true } },
    { text: '-', options: { color: COLORS.grayText, italic: true } },
    { text: '-', options: { color: COLORS.grayText, italic: true } },
    { text: '-', options: { color: COLORS.grayText, italic: true } },
    { text: '-', options: { color: COLORS.grayText, italic: true } }
  ...
  const taxTerms = [
    ...
    { term: 'Payroll Taxes', def: 'Employer-side payroll taxes (FICA, SUTA, FUTA) generated by direct, indirect, and induced employment. (Coming soon)' },
    { term: 'Income Taxes', def: 'State and federal income taxes paid by workers in direct, indirect, and induced jobs. (Coming soon)' }
  ];
